...
## 2026-08-05 — fix: 2ГИС реально ограничивает page_size 1-10, не 20/50
Статус: **завершён**

Найдено живым smoke-тестом объединённого поиска (см. запись выше) — `TwoGisPlacesService`
(закоммичен ранее, `4bae950`) отправлял `page_size` по умолчанию 20, а `SearchPlacesDto`
в api-gateway разрешал клиенту передать вплоть до 50. 2ГИС на самом деле сейчас отвечает:
`Length of parameter 'page_size' should be from 1 to 10`. Это не регрессия этой сессии —
преэкзистинг баг, ранее не пойманный, потому что unit-тесты мокают fetch и не бьются о
реальный лимит API.

Фикс: `TwoGisPlacesService.searchPlaces` — дефолт `pageSize ?? 20` → `?? 10`.
`SearchPlacesDto.pageSize` — `@Max(50)`/дефолт `20` → `@Max(10)`/дефолт `10`. Явно
переданный клиентом `pageSize` больше 10 по-прежнему пройдёт до 2ГИС и получит ошибку
от самого 2ГИС (сервис — тонкий клиент, не валидирует сверх того, что уже проверяет DTO
на границе api-gateway — тот же принцип, что и для `location`/`regionId`).

Проверки: `tsc --noEmit` — 0; `yarn test` — 102/102 (обновлён 1 существующий тест на
новый дефолт); `nest build` api-gateway + integration-service — success. Живой smoke
повторно пройден после фикса — `failedSources: []`, 2ГИС отвечает нормально.
